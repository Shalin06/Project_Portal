...
import { ref, onValue, update, set, get, remove,child } from "firebase/database";
...
const Details = () => {
  ...
  const Form = () => {
        ...
        <nav className="navbar">
          ...
          <div className="navbar__container">
            ...
            <ul className="navbar__links">
              {Profession === 'Professor' ? (
                <li><Link to="/profhome" style={{ textDecoration: 'none' }} className="hover1">Home</Link></li>) :
                (
                  <li><Link to="/studenthome" style={{ textDecoration: 'none' }} className="hover1">Home</Link></li>
                )}
              <li>
                <Link to="/Details" style={{ textDecoration: 'none' }} className="hover1"><FaUserCircle /> {username}</Link>
              </li>
              {Profession === 'Professor' ? (
                <li><Link to="/profProject" style={{ textDecoration: 'none' }} className="hover1">Add Projects</Link></li>) :
                (
                  <li><Link to="/StudentProj" style={{ textDecoration: 'none' }} className="hover1">Available Projects</Link></li>
                )}
              <li>
    ...
    const handleSubmit = async (event) => {
      ...
      const refff = ref(database, "users/" + user.uid);
      await get(refff).then(async (snapshot) => {
        if (snapshot.exists()) {
          if (snapshot.val().email) {
            await remove(child(refff, "email"));
            console.log("removed");
          }
        }
      });
    ...
    return (
      <>
        ...
        <div className="container2">
          <nav>
            <div className="login_navbar02">
              <form className="deatil_form">
                <div>
                  <label className="detail_font">Details Form</label>
                </div>
  ...
  return (
    <>
      <Form />
      <div className="animation_details">
